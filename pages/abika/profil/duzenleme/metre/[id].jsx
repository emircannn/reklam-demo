...
const index = ({product}) => {
  ...
  const [afterprint, setAfterprint] = useState(product.afterprint)
  ...
  const handleUpdate = async (id) => {
    ...
    try {
      ...
      if (res.status === 200) {
        toast.success("Ürün Başarıyla Güncellendi", {autoClose: 1000});
        sendServer()
        sendServer2() 
        sendServer3()
        setSuccess(true)
        setTimeout(() =>{setSuccess(false)}, 1500)
        setTimeout(() =>{push("/abika/profil")}, 1500)
      }
    } catch (err) {
      console.log(err);
  ...
  return (
    <div className='p-16 max-md:p-8 max-sm:p-4 relative'>
      <Head>
        ...
        <h1 className='font-bold text-2xl max-2xl:text-lg tracking-wide text-primary border-b-2 border-primary'>{product.title} Düzenleme</h1>
        <div className='mt-8 flex gap-8 max-md:flex-col'>
          ...
          {/* Sol Taraf */}
            ...
            <div className='flex items-center justify-center gap-4 w-full'>
            <button onClick={() => push("/abika/profil")} className='mt-8 w-full button'>Ana Sayfa</button>
            <button onClick={() => handleUpdate(product._id)} className='mt-8 w-full button'>Ürünü Güncelle</button>
            </div>
