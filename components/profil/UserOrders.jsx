...
const UserOrders = ({user}) => {
  ...
  useEffect(() => {
    ...
    const myOrders = orders.filter((order) => {return order?.email === user.email})
  ...
  return (
    <section className='md:p-8 lg:mt-0 mt-5 max-md:px-4 flex-1 relative'>
        ...
        <div className='mt-5 grid grid-cols-2 max-lg:grid-cols-1 gap-2'>
            {myOrders.sort((a,b) => {return new Date(b.createdAt) - new Date(a.createdAt)}).map((order) => (
                ...
                <div className='h-full flex items-end justify-end'>
                    <button onClick={() => push(`/profil/siparis/${order._id}`)} className='button'>Detaylar</button>
                </div>
            </div>
            ))}    
        </div> : <div className='w-full mt-8'><h2 className='font-bold text-center text-primary text-xl max-2xl:text-base'>Henüz Bir Siparişiniz Vermediniz...</h2></div>}
    </section>
  )
}
